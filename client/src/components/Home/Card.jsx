...
import { setPkmnsStats } from '../../actions/setters.actions';
import { SERVER_URL as SERVER_MINE } from '../../actions/constants.action';
...
function Card({ name, pokeId }) {
            ...
            if (Number.isInteger(Number(pokeId))) {
                ...
                dispatch(
                    setPkmnsStats({
                        ...
                        sprite: res.data.spriteSrc,
                        types: res.data.types.map((el) => el.name),
                        height: res.data.height,
                        weight: res.data.weight,
                    })
                );
                setSprite(res.data.spriteSrc);
    ...
    return (
        <div className={`${styles.container} ${styles[type[0]]}`}>
            ...
            <img className={styles.sprite} src={sprite} alt='Pokemon sprite.' />
            ...
            {type.length > 1 ? (
                ...
                <article className={styles.typeSingle}>
                    <h4>{type[0]}</h4>
